...
univ_gdf = gpd.GeoDataFrame(univ_df, geometry="geometry", crs="EPSG:4326").to_crs(
    taxi_zones.crs
)
unis_per_zone = (
...
weather_dd["time"] = dd.to_datetime(weather_dd["time"]).dt.round("h")
taxi_data["is_holiday"] = is_holiday(taxi_data["tpep_pickup_datetime_rounded"])

df = (
    taxi_data.merge(zones_dd, left_on="PULocationID", right_on="LocationID", how="left")
    .merge(
        zones_dd,
    ...
    .merge(
        weather_dd, left_on="tpep_pickup_datetime_rounded", right_on="time", how="left"
    )
)
