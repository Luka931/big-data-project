...
LOCATION_TO_BOROUGH = (
    pd.read_csv("location_to_borough.csv").set_index("LocationID")["Borough"].to_dict()
)
...
borough_stats_table = app.Table(
    "borough_stats", default=StatsData, key_type=str, value_type=StatsData, partitions=1
).tumbling(
...
def calculate_and_format_stats(stats, name):
    ...
    std_pass = math.sqrt(
        max(0, (stats.passenger_sq_sum / stats.count) - (mean_pass**2))
    )
...
async def process_taxi_ride(stream):
    async for ride in stream:
        ...
        feature_buffer.append([ride.distance, ride.base_fare, ride.passenger_count])
...
async def print_stats():
    ...
    if stats.count > 0:
        await rolling_stats_topic.send(
            value={
                ...
                "std_distance": math.sqrt(
                    max(
                        0,
                        (stats.distance_sq_sum / stats.count)
                        - ((stats.distance_sum / stats.count) ** 2),
                    )
                ),
                "mean_fare": stats.fare_sum / stats.count,
                "std_fare": math.sqrt(
                    max(
                        0,
                        (stats.fare_sq_sum / stats.count)
                        - ((stats.fare_sum / stats.count) ** 2),
                    )
                ),
                "mean_passengers": stats.passenger_sum / stats.count,
                "std_passengers": math.sqrt(
                    max(
                        0,
                        (stats.passenger_sq_sum / stats.count)
                        - ((stats.passenger_sum / stats.count) ** 2),
                    )
                ),
            }
        )
